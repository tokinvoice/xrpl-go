...
> **Note**: The definitive Go implementation is in `xrpl/ledger-entry-types/vault.go`.
> The snippet below is a simplified illustration; refer to the actual source file for accurate types and JSON tags.

```go
// File: xrpl/ledger-entry-types/vault.go (simplified)
type Vault struct {
    ...
    Asset             Asset           // currency/issuer object
...
- `binary-codec/types/number_test.go` – Tests for the `Number` serialized type (FromJSON, ToJSON, round-trip, edge cases)
- `binary-codec/types/issue_test.go` – Tests for the `Issue` type (XRP, IOU, MPT issuance ID, error handling)
- `binary-codec/vault_encode_test.go` – Tests for vault transaction encoding
...
**Implemented test files:**
- `xrpl/transaction/integration/vault_test.go` – Vault lifecycle tests (VaultCreate, VaultDeposit, VaultWithdraw, VaultSet)
- `xrpl/transaction/integration/loan_broker_test.go` – LoanBroker tests (LoanBrokerSet, LoanBrokerCoverDeposit, LoanBrokerCoverWithdraw)

**Running integration tests:**
```bash
# Run vault integration tests against lending devnet
INTEGRATION=lendingdevnet go test -v ./xrpl/transaction/integration/... -run TestIntegrationVault

# Run loan broker integration tests against lending devnet
INTEGRATION=lendingdevnet go test -v ./xrpl/transaction/integration/... -run TestIntegrationLoanBroker
```
...
## Appendix C: Outstanding Tasks / Future Work

The initial implementation of XLS-65 (Vault) and XLS-66 (Lending Protocol) is complete and validated against the lending devnet. The following items are intentionally left for follow-up work and refinement.

### C.1 Client Behaviour and Transaction Waiting

- [ ] **Align RPC `WaitForTransaction` with WebSocket semantics**
  - File(s): `xrpl/rpc/helpers.go`, `xrpl/websocket/client.go`.
  - Goal: Make `Client.WaitForTransaction` over RPC behave as closely as possible to the WebSocket version:
    - Poll `tx` first and **return immediately when `Validated == true`**.
    - Use `LastLedgerSequence` and current ledger index only as a timeout/expiry bound.
  - Add or extend tests to cover successful validation, expiry (no transaction found before `LastLedgerSequence`), and error paths.

### C.2 Ledger Entry Types and Documentation

- [ ] **Clarify `WithdrawalPolicy` semantics for Vaults**
  - Files: `xrpl/ledger-entry-types/vault.go`, this design doc.
  - Replace any hard-coded numeric interpretation comments (for example, "0 = instant, 1 = stranded") with wording that defers to the XLS-65 spec unless/until there is a stable, documented mapping from Ripple.
  - Ensure the field description in the table under _Part 1: Vault_ matches the implementation and spec ("withdrawal strategy code; see XLS-65 docs" is a safe baseline).

- [ ] **Bring example Go structs in this document in line with the actual code**
  - Files: `xls-65-66.md`, `xrpl/ledger-entry-types/vault.go`, `xrpl/ledger-entry-types/loan_broker.go`, `xrpl/ledger-entry-types/loan.go`.
  - The struct sketches in this doc (e.g. `OwnerNode string`, `Asset Currency`, `MPTokenIssuanceID string`) were written early and may not exactly match the final Go types and JSON tags.
  - Update the code blocks under the Vault / LoanBroker / Loan sections to mirror the concrete implementations (types, JSON tags, field ordering), or explicitly label them as illustrative pseudo-code.

### C.3 Integration Tests and Comments

- [ ] **Make vault and loan-broker integration test comments match behaviour**
  - Files: `xrpl/transaction/integration/vault_test.go`, `xrpl/transaction/integration/loan_broker_test.go`.
  - Current comments describe slightly richer flows (e.g. vault deletion, broker deletion) than what the tests currently execute.
  - Either:
    - Adjust the comments to describe only the implemented steps, **or**
    - Extend the tests to actually perform the full flow (e.g. add `VaultDelete` and `LoanBrokerDelete` transactions to the happy-path scenarios).

- [ ] **Add broader lending integration coverage (optional but recommended)**
  - Files: `xrpl/transaction/integration/loan_broker_test.go` (new tests may live in a separate file if preferred).
  - Suggested flows to exercise against lending devnet:
    - `LoanSet` end-to-end with a minimal borrower/lender setup, including the `Counterparty` / `CounterpartySignature` pattern if/when multi-sign orchestration is available in Go.
    - `LoanManage` flag transitions (default, impair, unimpair) on an existing loan.
    - `LoanPay` covering at least an on-time payment and a late/default-case payment where possible.
    - `LoanBrokerCoverClawback` for a simple clawback scenario.
  - Follow existing patterns used in the vault and loan-broker tests (use `Runner`, `TestTransaction`, and `TestTransactionAndWait`).

### C.4 Codec and Type-Level Testing

- [ ] **Harden tests for the `Number` serialized type**
  - File: `binary-codec/types/number.go` and a new/extended test file under `binary-codec/`.
  - Add focused unit tests for:
    - Values near the minimum/maximum allowed exponents.
    - Negative mantissas and negative exponents.
    - Boundary behaviour around the decimal vs scientific rendering thresholds used in `ToJSON`.
    - Canonical zero representation (mantissa 0 with `DefaultValueExponent`).

- [ ] **Add tests for `Issue` self-describing logic (XRP / IOU / MPT issuance ID)**
  - File: `binary-codec/types/issue.go` and corresponding tests.
  - Cover the three main cases:
    - All-zero currency bytes → XRP.
    - Currency + non-`NoAccountBytes` issuer → IOU.
    - Currency + `NoAccountBytes` + sequence → MPT issuance ID (`mpt_issuance_id`); include at least one negative and one positive sequence test.
  - Include error-case tests for malformed lengths or unexpected byte patterns.

### C.5 Documentation and Developer Experience

- [ ] **Document `txNeedsNetworkID` version logic more explicitly**
  - Files: `xrpl/rpc/helpers.go`, possibly this design doc.
  - Add short comments clarifying that `RequiredNetworkIDVersion` is the minimum rippled version that requires NetworkID for restricted networks (currently `1.11.0`), and that `txNeedsNetworkID` is intended to match rippled's behaviour on those networks.

- [ ] **Optionally expand "Testing Strategy" with direct links to actual test files**
  - File: `xls-65-66.md`.
  - Under _Part 6: Testing Strategy_, add bullet points pointing to the concrete Go tests now in the repo (e.g. `xrpl/transaction/integration/vault_test.go`, `xrpl/transaction/integration/loan_broker_test.go`), so future contributors know where to extend coverage.
